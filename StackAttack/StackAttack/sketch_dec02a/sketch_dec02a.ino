...
private:
  ...
  bool UpdatePlayer()
  {
    ...
    case 'w':
      ...
      if(sceneGrid[player.x+1][player.y] == '#' || (player.x) == rows - 1)
      { 
        if (sceneGrid[player.x - 1][player.y] == '#') // Åñëè ñâåðõó áëîê - ðàçáèâàåì
        {
          sceneGrid[player.x - 1][player.y] == '*';
          player.x -= 1;
          return true;
        }
        else  // ïðîñòî çàâèñàåì
        {
          player.x -= 1;
          return true;
        }
      }
      else if (sceneGrid[player.x + 1][player.y] == '*') // ñíèçó ïóñòî - ïàäàåì
      {
        player.x += 1;
        return true;
      }
    ...
    case 'd':
      // player moves right
      // check for blocks & try to move block & move player
      if (!(player.y < cols - 1)) // Åñëè õîòèì ïðîñòî ñäâèãàòü â áîê çà ïîëå
        return true;
      
      playerSideBottom = sceneGrid[player.x][player.y + 1]; // ïðàâàÿ êëåòêà
       playerSideUp = sceneGrid[player.x - 1][player.y + 1]; // ãîëîâà ïðàâîé êëåòêè
    ...
    case 'a':
      ...
      if (playerSideBottom == '*') // ðÿäîì ïóñòî
      {
        if (playerSideUp == '*')  // ñâåðõó ïóñòî
        {
          if (sceneGrid[player.x + 1][player.y - 1] == '*')
          {
            player.x += 1;
